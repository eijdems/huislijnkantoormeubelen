<?php
namespace Emizen\Download\Model\Customer;
...
class EmailNotification extends MagentoEmailNotification
{
    ...
    {   
        /*$writer = new \Zend_Log_Writer_Stream(BP . '/var/log/emailcustom1232323.log');
        $logger = new \Zend_Log();
        ...
        if (isset($customerData['custom_attributes'])) {
            foreach ($customerData['custom_attributes'] as $attribute) {
                 $mergedCustomerData->setData($attribute['attribute_code'], $attribute['value']);
            }
        }
       
        //$logger->info('Customer number: ' . $customerNumber);
    ...
    public function newAccount(
        ...
        $customerEmailData = $this->getFullCustomerObject($customer);
        //$logger->info(print_r($customerEmailData,true));
        $this->sendEmailTemplate(
            $customer,
            $types[$type],
